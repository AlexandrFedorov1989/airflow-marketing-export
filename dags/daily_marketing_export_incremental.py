# тот же пайплайн, что и full, но mode=incremental
# берём last_successful_ts из StateStore и тянем только новые записи
...
DAG_ID = "daily_marketing_export_incremental"
...
with DAG(
    ...
    description="Incremental marketing events export",
    schedule="@daily",
    start_date=datetime(2026, 6, 10),
    catchup=False,  # старые даты назад не гоняем
    max_active_runs=1,
    tags=["marketing", "export", "incremental"],
) as dag:
    ...
    start_export = MarketingExportOperator(
        ...
        output_path=build_export_path(
            DATA_BASE_DIR,
            "{{ ds }}",
            filename="export_incremental.jsonl",
        ),
    ...
    wait_export_ready = MarketingExportSensor(
        ...
        marketing_conn_id=MARKETING_CONN_ID,
        mode="reschedule",  # не держим слот воркера между poke
        poke_interval=30,
        timeout=60 * 60,
    )
    ...
    def verify_file_callable(**context) -> int:
        # файл должен существовать, быть непустым и читаться как JSONL
